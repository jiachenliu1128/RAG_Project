...
from self_rag import should_retrieve
...
def answer_query_with_context(
    ...
    Args:
        ...
        reranker: Reranker instance for re-ranking results (optional)
        initial_k: Number of candidates to retrieve before re-ranking (default 20)
        rewrite: If True, rewrite query before retrieval
        retrieval_decision: If True, LLM decides whether to retrieve
        show_prompt: If True, log the constructed prompt
        
    Returns:
        Tuple of (answer, prompt)
    """
    ...
    # Step 1: Decide if retrieval is needed (optional)
    if retrieval_decision:
        logger.info("Checking if retrieval is needed...")
        # Prefer ML decider if available by passing faiss_backend through self_rag.should_retrieve
        should_ret, retrieval_reasoning = should_retrieve(query, faiss_backend=faiss_backend)
        if not should_ret:
            logger.info("Retrieval not needed - generating answer directly")
            response = client.chat.completions.create(
                messages=[{"role": "user", "content": f"Answer this question directly: {query}"}],
                **COMPLETIONS_API_PARAMS
            )
            direct_answer = response.choices[0].message.content.strip(" \n")
            return (direct_answer, f"Direct answer (no retrieval): {query}")
    
    # Step 2: Rewrite query if enabled
    effective_query = rewrite_query(query) if rewrite else query
    if rewrite and effective_query != query:
        logger.info(f"Query rewritten: '{query}' → '{effective_query}'")
    
    # Step 3: Construct prompt with retrieval
